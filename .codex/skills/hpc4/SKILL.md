...
この同居は実は VPN / network client が起動時に kernel routing table に書き込む結果として **OS が自然に成立させる**：default route は通常通信、HKUST 向け route は HKUST 側接続、longest-prefix-match で衝突なし。skill 自身は default route や Codex の通信経路に絶対手を出さず、**HPC4 (143.89.184.3) の host route 1 つだけを管理する**。
...
この `.codex/` skill は Codex 実行を前提にする。Codex の通常 sandbox では macOS の route socket / TCP probe / BatchMode SSH が高確率で判定不能になるため、HPC4 に関わる観測・SSH・転送 command は **初回から `require_escalated` で実行する**。通常 sandbox で 1 回失敗させてから escalation するのは診断ではなく、既知の false negative を再生しているだけなのでやらない。

権限境界は 2 種類ある。混同しない：

1. **Codex sandbox の read/probe 制限**：`route get`、`ping`、`nc`、BatchMode SSH などが `Operation not permitted` 系で false negative になる状態。これは host route や pf を変更する操作ではない。`status.sh`、`net-up.sh`、`net-down.sh`、`ssh-run.sh`、`xfer.sh` のように経路・疎通・SSH 到達性を観測する command は、Codex では最初から `require_escalated` で呼ぶ。
...
| `net-up-local.sh` | なし | Codex は直接呼ばない。route / pf 操作に sudo が必要な時、または SSH key passphrase 入力が必要な時に、user が自分の Terminal で実行する helper。L3 正常 + TCP 22 不通なら pf main ruleset flush を 1 回だけ自動実行し、経路 OK 後に passwordless SSH 未成立なら候補鍵を選ばせて `ssh-add` まで実行する |
...
2. `[?] route socket 制限` または egress / TCP 22 / passwordless SSH が `[?]` → 通常 sandbox で誤実行してしまった事故として扱う。`require_escalated` で同じ `status.sh` を実行し直す。拒否・不可の場合だけ、別ターミナルで同じ script を実行してもらう
3. `[ng] HKUST 圏内 IF なし` → status.sh が出した日本語の指示（「HKUST キャンパス内 eduroam か HKUST 有線、またはオフキャンパスなら Ivanti Secure Access を起動してください」）をそのまま提示。修正後に再開するよう案内し、setup を **ここで止める**（ssh-copy-id まで進めない）
...
6. ネットワーク `[ok]` + passwordless SSH 未成立 → **setup ステップ C を実行**（まず `net-up-local.sh` で候補鍵の選択と `ssh-add`、それでも未成立なら選択鍵に対応する `ssh-copy-id` 案内）
...
3. **VPN 製品の判別はしない**: ifconfig で「HKUST 内部 IPv4（`143.89/16` または HKUST eduroam の `10.79/16`）を持つ IF があるか」だけで判定する。製品名では分岐しない
...
6. **Codex の HPC4 script 呼び出しは初回から escalation**: `[?]` は unknown であり、失敗 verdict ではない。Codex sandbox では route / TCP / SSH probe が既知に判定不能になるため、`status.sh`、`net-up.sh`、`net-down.sh`、`ssh-run.sh`、`xfer.sh` は最初から `require_escalated` で実行する。通常 sandbox の `[?]` を見てから行動を決める設計にしない。素の Bash コマンド例を見ても、Codex tool 呼び出しでは `sandbox_permissions="require_escalated"` を付ける。
7. **sudo と SSH key passphrase は user Terminal helper に委任**: Codex が直接呼ぶ scripts は sudo prompt や passphrase prompt を扱わない。route / pf 操作や `ssh-add` が必要な時は user に `bash .codex/skills/hpc4/scripts/net-up-local.sh` を自分の Terminal で実行してもらう。helper 内では route 修正、pf main ruleset flush、SSH agent への鍵追加を自動実行してよい。Codex 配下では sudo password / Touch ID prompt や passphrase prompt が silent fail し、呼び出し側が「成功した」と誤認する事故が起きるため
...
- **ネットワーク**: HKUST 内部 IPv4（有線 / Ivanti の `143.89/16`、または HKUST キャンパス eduroam の `10.79/16`）を持つ IF が 1 つでもあれば動く。具体的には：HKUST キャンパス内 eduroam、HKUST 有線、Ivanti Secure Access (HKUST SSL VPN)。**HKUST 以外の eduroam（DT Hub 等の HKSTP 施設、他大学、空港）は対象外** — 同じ SSID で繋がるが HKUST 内部 IP は降ってこない（[policy.md](policy.md) 参照）。default route を別 VPN が握っていても、その VPN の kill-switch / packet filter / NEPacketTunnelProvider が 143.89.184.3 行きを遮断していなければ HPC4 だけは抜けられる
...
- `[?] route socket 制限`、`[?] HPC4 egress`、`[?] TCP 22`、`[?] passwordless SSH` → 通常 sandbox で誤実行してしまった事故として扱う。`status.sh` を `require_escalated` で実行し、`[ok]` なら続行、`[ng]` ならその `[ng]` の分岐に従う。escalation が拒否・不可の場合だけ、別ターミナルで同じ絶対パスの script を実行してもらう
...
- `[ng] TCP 22 不通` → `bash .codex/skills/hpc4/scripts/net-up.sh` を `require_escalated` で実行して詳細診断。Codex 直実行で sudo が必要なら `net-up-local.sh` を案内する。user Terminal helper 内で L3 正常 + TCP 22 不通になった場合は pf main ruleset flush を 1 回だけ自動実行し、flush 後も復旧しなければ user space から復旧不能として止める
...
3. **未成立の場合** はまず `net-up-local.sh` を user の Terminal で実行してもらう。`HPC4_IDENTITY_FILE` が設定済みならその鍵を使い、未設定なら `~/.ssh/` から候補鍵を提示して選ばせる。暗号化された秘密鍵が agent 未登録なだけなら、この helper が `ssh-add` を実行し、Terminal 上で passphrase を入力させてから再判定する。
4. `net-up-local.sh` 後も未成立の場合だけ、以下を 1 回だけ提示：
   - 秘密鍵がなければ `ssh-keygen -t ed25519 -N "" -f ~/.ssh/id_ed25519` を自動実行してよい（BatchMode 可）
   - 公開鍵未登録なら、`net-up-local.sh` が出す選択鍵に対応した `ssh-copy-id -i <selected-key>.pub <HPC4_USER>@hpc4.ust.hk` を「別ターミナルで 1 回だけ実行してください（password + 2FA の入力が必要）」と案内
   - 完了したらチャットで知らせてもらい、再度 status.sh で成立確認
5. 成立したら setup 完了メッセージを出す。

### ステップ D. 動作確認

`bash .codex/skills/hpc4/scripts/ssh-run.sh 'hostname && whoami'` を `require_escalated` で走らせて、`login{N}` と自分の username が返ってくることを確認。

## 典型的な呼び出しパターン

Codex が shell から叩く際のテンプレート（パスは working directory 起点）。下の code block は Bash コマンド文字列だけを示す。**Codex tool 呼び出しでは、`write-user-conf.sh` 以外の HPC4 観測・SSH・転送 command に必ず `sandbox_permissions="require_escalated"` を付ける**：
...
**判定の本質：HKUST 内部 IPv4（`143.89/16` または HKUST eduroam の `10.79/16`）を持つ IF がこの Mac に 1 つでもあるか**。製品名で分岐しない。ifconfig を直接読む。
...
Codex が直接呼ぶ scripts は sudo prompt や SSH key passphrase prompt を扱わない。route / pf 操作（host route add/delete、pf main ruleset flush）または暗号化鍵の `ssh-add` が必要な時は、scripts が `bash .codex/skills/hpc4/scripts/net-up-local.sh` を案内するので、user が自分の Terminal でそれを実行する。`net-up-local.sh` は interactive sudo を許可し、L3 正常 + TCP 22 不通では `pfctl -F rules` を 1 回だけ自動実行する。経路 OK 後、passwordless SSH が未成立なら `HPC4_IDENTITY_FILE` の明示鍵、または `~/.ssh/` から選択された候補鍵を `ssh-add` し、Terminal 上で passphrase を入力させてから再判定する。
...
なぜ Codex 実行中の sudo / passphrase 入力を避けるか：Codex の shell 実行では sudo password / Touch ID prompt や SSH key passphrase prompt が user に届かない、または入力できない。route / pf 操作と `ssh-add` は user Terminal helper に委任することで、Codex に相談できない時でも user が terminal だけで完結できる設計になる。

## トラブル時の当たり方

1. `bash .codex/skills/hpc4/scripts/status.sh` を `require_escalated` で実行して現状を取り、どの層で落ちているか特定（ネットワーク / 認証 / ControlMaster）
2. `bash .codex/skills/hpc4/scripts/net-up.sh` を `require_escalated` で実行して経路を診断。sudo が必要なら user に `bash .codex/skills/hpc4/scripts/net-up-local.sh` を実行してもらう
3. `bash .codex/skills/hpc4/scripts/net-down.sh` を `require_escalated` で実行した後、`net-up.sh` を `require_escalated` で実行して完全リセット
4. それでも繋がらないなら `ssh -F .codex/skills/hpc4/ssh_config -l <user> -vvv hpc4` を `require_escalated` で直接叩いて詳細ログを取る
...
`nohup ... & disown` の生存性は OS / login shell / SSH client の組み合わせに依存し、SIGHUP が漏れて落ちるケースが実測される。安全側に倒すため、**数時間以上回す dispatcher は tmux/screen の detached session** に入れる。Codex tool 呼び出しでは `ssh-run.sh` を `require_escalated` で実行する:
...
3. **ルートが再起動・スリープで消える**: macOS の host route は永続化されない。スリープ復帰時は `bash .codex/skills/hpc4/scripts/net-up.sh` を `require_escalated` で再実行（冪等）
...
すべて `bash .codex/skills/hpc4/scripts/ssh-run.sh '<HPC4 上で実行する 1 行>'` の形。Codex tool 呼び出しではすべて `require_escalated` で実行する：
