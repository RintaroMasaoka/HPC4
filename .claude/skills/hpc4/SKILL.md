...
Claude (Anthropic API) は香港をサービス対象外としており、一方 HPC4 は HKUST のネットワーク（143.89/16 IP 帯）からしか到達できない。**Claude を使うには香港外を経由する必要があり、HPC4 を使うには HKUST 圏内に居る必要がある**。同じ Mac 上で「Claude 用の経路（NordVPN 等で日本／第三国経由）」と「HPC4 用の経路（HKUST 直結 or Ivanti split-tunnel）」を同居させるのが運用の前提。
...
下表の scripts はすべて `bash .claude/skills/hpc4/scripts/<name>.sh [args]` の形で Bash ツールから直接叩ける。**`ssh-run.sh` / `xfer.sh` は ping が通れば直接 ssh、通らなければ `net-up.sh` を呼んで経路を診断する**ので、経路 OK のときは経路を意識せず目的の操作を直接呼んでよい。`net-up.sh` が user による sudo を要求した場合は exit 3 で抜けるので、その場合のみ案内を user に伝える。
...
| `net-up.sh` | なし | 経路を診断したい時、status で TCP 22 不通を発見した時。**route 操作 (sudo) は user に委任する**ので、出力に「sudo route ... を別ターミナルで実行してください」が出ることがある |
...
3. `[ng] HPC4 host route：... stale pin` または `[ng] HPC4 egress：... HKUST 圏外` → status.sh が出した `sudo route ...` の 1 行を user に「別ターミナルで実行してください」とそのまま提示。完了したら status.sh から再開
4. `[ng] TCP 22 不通` → `bash .claude/skills/hpc4/scripts/net-up.sh` を呼んで詳細診断する。L4 遮断の場合は VPN クライアント GUI 設定の指示をそのまま提示
...
2. **触る対象は HPC4 host route ただ 1 つ**: 143.89.184.3 の host route だけを操作対象とする（実行は user 委任）。default route や Claude 経路には絶対に手を出さない
3. **VPN 製品の判別はしない**: ifconfig で「143.89/16 IP を持つ IF があるか」だけで判定する。NordVPN だの Ivanti だの製品名で分岐しない
...
6. **sudo は user 委任**: scripts は内部で sudo を呼ばない。route 操作が必要な時は「別ターミナルで `sudo route ...` を実行してください」と user に正確な 1 行を提示する。Claude Code 配下では sudo password / Touch ID prompt が silent fail し、呼び出し側が「成功した」と誤認する事故が起きるため
...
- **ネットワーク**: 143.89/16 (HKUST) IPv4 を持つ IF が 1 つでもあれば動く。具体的には：HKUST キャンパス内 eduroam、HKUST 有線、Ivanti Secure Access (HKUST SSL VPN)。**HKUST 以外の eduroam（DT Hub 等の HKSTP 施設、他大学、空港）は対象外** — 同じ SSID で繋がるが HKUST IP は降ってこない（[policy.md](policy.md) 参照）。default route を別 VPN（NordVPN 等）が握っていても、その VPN の kill-switch / packet filter / NEPacketTunnelProvider が 143.89.184.3 行きを遮断していなければ HPC4 だけは抜けられる
...
- `[ng] HPC4 host route：... stale pin` → status.sh が出した `sudo route -n delete -host ...` を user に「別ターミナルで実行してください」と提示。完了したら再開
- `[ng] HPC4 egress：... HKUST 圏外` → status.sh が出した `sudo route -n add -host ... -interface ...` を user に「別ターミナルで実行してください」と提示。完了したら再開
- `[ng] TCP 22 不通` → `bash .claude/skills/hpc4/scripts/net-up.sh` で詳細診断。L4 遮断の場合は VPN クライアント GUI 設定の手順を出す

要点：scripts は sudo を呼ばず、必要な sudo コマンドを文字列として user に提示する（user 委任）。
...
**判定の本質：143.89/16 (HKUST) IPv4 を持つ IF がこの Mac に 1 つでもあるか**。製品名（NordVPN / Ivanti / eduroam ...）で分岐しない。ifconfig を直接読む。

### アルゴリズム（`net-up.sh` / `status.sh` 共通の core）

1. **`find_hkust_iface`：ifconfig を全 IF 走査して 143.89/16 IPv4 を持つ IF を探す** — routing と独立した「HKUST 圏内能力」の判定。stale な host pin が `route get` を歪めても、ここは騙されない
   - 無ければ → 「HKUST 圏に入れ」（HKUST キャンパス eduroam / 有線 / Ivanti）。pin 操作は無駄なので即終了
2. **stale pin 検出**：`current_hpc4_iface` で既存の host pin を見て、HKUST 圏外 IF を指していたら → user に `sudo route -n delete -host ...` を委任。stale pin 起因の誤診断と「圏外」を別 path で報告するのが key
3. **`route get 143.89.184.3` で kernel の自然な egress を見る** — HKUST 圏外 IF を選んでいる稀ケースなら user に `sudo route -n add -host ... -interface <hkust_iface>` を委任
...
- HKUST の end-user ネット（HKUST キャンパス eduroam / 有線 / Ivanti）は **143.89/16 を直接配布する**ので、IF が 143.89/16 IPv4 を持っているか否かで「HKUST 圏に居るか」を一意に判定できる。federated eduroam（DT Hub 等）はここで自動的に弾かれる
...
scripts は内部で sudo を呼ばない。route 操作（add / delete）が必要な時は、scripts が「別ターミナルで以下を実行してください」と sudo コマンドの 1 行を出力するので、user がコピペで叩く。

なぜ scripts 内 sudo を避けるか：Claude Code の Bash ツール経由では sudo password / Touch ID prompt が user に届かず silent fail し、「成功した」と script が誤認する事故が起きる（実際にこの skill の旧版がそれで stale pin の誤診断を引き起こした）。route 操作は user 委任にすることで、Claude が落ちている時でも user が terminal だけで完結できる設計になる。
