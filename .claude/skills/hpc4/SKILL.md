...
4. `[ng] TCP 22 不通` → `bash .claude/skills/hpc4/scripts/net-up.sh` を呼んで詳細診断する。最有力は NordVPN 系の pf `block drop all` で、`sudo pfctl -F rules` を user に依頼する案内が出る
...
2. **触る対象は HPC4 宛 host route だけ**: 操作対象は 143.89.184.3 の host route 1 つに限定。実行は user 委任。pf rules には触らない（kill-switch 系 VPN が main ruleset に書く `block drop all` には anchor 経由の pass quick が届かないと判明したため、`com.apple/hpc4` anchor 戦略は撤去済み）。default route や Claude 経路にも絶対に手を出さない
...
4. **L4 で塞がれている時は user に `sudo pfctl -F rules` を案内する**: NordVPN 等の helper が pf main ruleset に書く `block drop all` 系 kill-switch は anchor 経由 pass quick で貫通できないため、main rules ごと flush するしかない。flush 後 NordVPN tunnel と Claude 経路はそのまま生きる。NordVPN を Disconnect/Connect すると rules が再注入されるので、その都度 flush。NEPacketTunnelProvider (`includeAllNetworks=true`) のシステム強制 tunnel など pf より下層の機構で塞がれている場合は user space から override 不可なので、当該 VPN クライアント側の設定見直しを案内する
5. **個人情報の隔離**: ITSO ユーザ名等は `user.conf.local`（gitignored）に閉じる。リポジトリ本体には書かない
6. **sudo は user Terminal helper に委任**: AI が直接呼ぶ scripts は sudo prompt を扱わない。route / pf 操作が必要な時は user に `bash .claude/skills/hpc4/scripts/net-up-local.sh` を自分の Terminal で実行してもらう。Claude Code 配下では sudo password / Touch ID prompt が silent fail し、呼び出し側が「成功した」と誤認する事故が起きるため
...
- `[ng] TCP 22 不通` → `bash .claude/skills/hpc4/scripts/net-up.sh` で詳細診断。最有力は NordVPN 系の pf `block drop all` で、`sudo pfctl -F rules` の案内が出る

要点：AI が直接呼ぶ scripts は sudo prompt を扱わない。route 変更や pf flush が必要な時は user Terminal で実行してもらう。
...
5. **L3 OK だが L4 で塞がれている**（NordVPN 系 helper の pf `block drop all` / NEPacketTunnelProvider 等）→ user に `sudo pfctl -F rules` を案内する（前者ならこれで通る）。後者なら user space から override 不可として止める
...
- L4 遮断のうち NordVPN 系 helper の pf `block drop all` 起因のものは `sudo pfctl -F rules` で解消する（これだけ user に案内する）。それ以外（NEPacketTunnelProvider 等）は user space からは復旧不能なので、skill 側では追加の回避策を提示しない

SSH 側は ControlMaster で 12h persist、password+2FA は実質 1 日 1 回で済む。

## sudo が必要な操作について

AI が直接呼ぶ scripts は sudo prompt を扱わない。route / pf 操作（add / delete / anchor 適用）が必要な時は、scripts が `bash .claude/skills/hpc4/scripts/net-up-local.sh` を案内するので、user が自分の Terminal でそれを実行する。
...
2. **VPN クライアントの kill-switch が L4 で 143.89.184.3 行きを遮断する**: NordVPN 系 helper の場合は pf main ruleset に `block drop all` を入れるタイプで、`sudo pfctl -F rules` で main rules ごと flush すれば通る（NordVPN tunnel と Claude 経路は無傷）。NEPacketTunnelProvider (`includeAllNetworks=true`) のシステム強制 tunnel は user space から override 不可なので skill 側からは復旧不能
