...
セットアップが済んでいれば、HPC4 がらみの作業はチャットで普通に頼むだけで Claude が裏側で経路整備・コマンド実行・ファイル転送を進める。例:
...
subcommand を覚える必要はない。`/hpc4` 一発で対話的に進む。
...
意図的に teardown したい場合は Claude に「経路を取り外して」と頼む（あるいは `/hpc4 down`）。冪等に anchor のルールを空にし、route を削除し、ControlMaster を close する。
...
- **他のキルスイッチ型 VPN client** は問題なく共存する（NordVPN / SurfShark / ExpressVPN / Proton / Mullvad で動作確認済）
...
<details>
<summary><b>subcommand 一覧</b>（直接叩く時の参照）</summary>

通常は Claude に自然言語で頼めばよく、subcommand を覚える必要はない。直接呼びたい時の参照:

| subcommand | 内容 |
|---|---|
| （引数なし） | 未セットアップなら setup を起動、セットアップ済みなら「何をしたいか」を聞く |
| `status` | 現在の接続状態を表示 |
| `setup` | 初回セットアップ（ITSO 名収集 + passwordless SSH の確認） |
| `up` | HPC4 に届く経路を整える（route + pf anchor、必要分だけ） |
| `down` | 経路と pf anchor を取り外す |
| `run '<command>'` | HPC4 上で任意コマンドを実行 |
| `put <local> <remote>` | ファイルを HPC4 にアップロード |
| `get <remote> <local>` | ファイルを HPC4 からダウンロード |
| `put-r <local> <remote>` | ディレクトリを rsync で送る |
| `get-r <remote> <local>` | ディレクトリを rsync で取得 |
