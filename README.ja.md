...
HKUST HPC4 クラスタへの SSH / コマンド実行 / ファイル転送を、ネットワーク経路の面倒ごと込みで自動化する Claude Code 用 skill。Mac (オンキャンパス / オフキャンパス + HKUST SSL VPN / 別フル VPN 併用) のどの状態でも同じ呼び方で繋がる。
...
- HKUST 圏に届く IF が無い（別 VPN が default を握っているだけ、または完全オフキャンパス）→ pin しても無駄なので即終了し、user に「Ivanti 起動 / 学内ネット切替」を案内
